...
import numpy as np
import pandas as pd
from megan.feat import ReactionFeaturizer

from megan.feat import ORDERED_ATOM_OH_KEYS, ORDERED_BOND_OH_KEYS

from megan.feat.graph_features import ATOM_PROPS, BOND_PROPS, ATOM_PROP2OH, BOND_PROP2OH
from rdkit import RDLogger
from scipy import sparse
from tqdm import tqdm

from megan.feat.featurize import featurize_parallel
from megan.feat.find_properties import find_properties_parallel
from megan.feat.mol_graph import unravel_atom_features, unravel_bond_features
from megan.split import DatasetSplit
from megan.utils import to_torch_tensor, lists_to_tuple
...
class MeganTrainingSamplesFeaturizer(ReactionFeaturizer):
    ...
    def get_actions_vocabulary(self, feat_dir: str) -> dict:
        if self._vocabulary is None:
            ...
            with open(get_actions_vocab_path(feat_dir), 'r') as fp:
                action_tuples = json.load(fp)

            # prop2oh_path = get_prop2oh_vocab_path(self.dir(feat_dir))
            prop2oh_path = get_prop2oh_vocab_path(feat_dir)
    ...
    def featurize_dataset(self, dataset):
        ...
        action_tuples = []
        sample_inds = []
        tmp = zip(chunk_inds, chunk_results, chunk_save_paths)
