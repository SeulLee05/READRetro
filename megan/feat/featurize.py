...
from megan.feat.mol_graph import get_graph
from megan.feat.reaction_actions import ReactionAction, AddRingAction, AddAtomAction, BondEditAction, AtomEditAction, \
    StopAction
from megan.feat.ring_actions import find_added_benzene_rings
from megan.feat.utils import atom_to_edit_tuple, get_bond_tuple, fix_incomplete_mappings, reac_to_canonical, \
    fix_explicit_hs
from megan.utils import renumber_atoms_for_mapping, mark_reactants
...
def featurize_parallel(params) -> int:
    ...
    for reaction_i, (reaction_ind, train, target_smi, source_smi) in \
        ...
        try:
            ...
            logger.info(f"fail smi: {target_smi, source_smi}")
        ...
        try:
            ...
            logger.info(f"fail samples: {training_samples, final_smi}")
            n_unparsed += 1
            continue
    ...
    if len(nodes_vals[0]) > 0:
        logger.info(f'shpae:{samples_len, max_n_nodes}, len node_vals:{len(nodes_vals[0]),len(nodes_vals[1]),len(nodes_vals[2])}')
        nodes_mat += sparse.csr_matrix((nodes_vals[0], (nodes_vals[1], nodes_vals[2])),
                                       shape=(samples_len, max_n_nodes))
        adj_mat += sparse.csr_matrix((adj_vals[0], (adj_vals[1], adj_vals[2])),
                                     shape=(samples_len, max_n_nodes ** 2))
    ...
    logger.info(f'sample_data shpae:{samples_len, n_sample_data}, len sample data:{len(sample_data[0])}')
